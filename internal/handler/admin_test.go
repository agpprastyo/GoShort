...
// mockAdminService adalah implementasi mock dari IAdminService untuk pengujian.
type mockAdminService struct {
	ListAllLinksFunc     func(ctx context.Context, req dto.GetLinksRequest) ([]dto.LinkResponse, *dto.Pagination, error)
	GetLinkByIDFunc      func(ctx context.Context, id uuid.UUID) (*dto.LinkResponse, error)
	ListUserLinksFunc    func(ctx context.Context, userID uuid.UUID, req dto.GetLinksRequest) ([]dto.LinkResponse, *dto.Pagination, error)
	ToggleLinkStatusFunc func(ctx context.Context, id uuid.UUID) error
}

// Memastikan mockAdminService memenuhi kontrak service.IAdminService.
var _ service.IAdminService = (*mockAdminService)(nil)

func (m *mockAdminService) ListAllLinks(ctx context.Context, req dto.GetLinksRequest) ([]dto.LinkResponse, *dto.Pagination, error) {
	return m.ListAllLinksFunc(ctx, req)
}

func (m *mockAdminService) GetLinkByID(ctx context.Context, id uuid.UUID) (*dto.LinkResponse, error) {
	return m.GetLinkByIDFunc(ctx, id)
}

func (m *mockAdminService) ListUserLinks(ctx context.Context, userID uuid.UUID, req dto.GetLinksRequest) ([]dto.LinkResponse, *dto.Pagination, error) {
...
func TestAdminHandler_ListAllLinks(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			queryParams: "",
			setupMock: func(mock *mockAdminService) {
				mock.ListAllLinksFunc = func(ctx context.Context, req dto.GetLinksRequest) ([]dto.LinkResponse, *dto.Pagination, error) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			handler := NewAdminHandler(mockService, newTestLogger())
...
func TestAdminHandler_GetLink(t *testing.T) {
	linkID := uuid.New()
	mockLink := &dto.LinkResponse{ID: linkID, ShortCode: "xyz", OriginalURL: "https://example.com/2"}
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupMock: func(mock *mockAdminService) {
				mock.GetLinkByIDFunc = func(ctx context.Context, id uuid.UUID) (*dto.LinkResponse, error) {
...
func TestAdminHandler_ListUserLinks(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupMock: func(mock *mockAdminService) {
				mock.ListUserLinksFunc = func(ctx context.Context, id uuid.UUID, req dto.GetLinksRequest) ([]dto.LinkResponse, *dto.Pagination, error) {
